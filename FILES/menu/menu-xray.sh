...
function login-limit() {
	...
	if ! [[ $new_limit =~ ^[0-9]+$ ]]; then
		echo ""
		echo "'$new_limit' is not a number."
		echo ""
		exit 1
	fi
	echo $new_limit > /donb/script-3/limit
...
function block-conn() {
	...
	if [ "$status" == on ]; then
		...
		if [ $option -eq 1 ]; then
			...
			ufw delete allow 3646 > /dev/null 2>&1
			echo on > /donb/block-status
		elif [ $option -eq 2 ]; then
			clear
			echo -e "Operation cancelled."
			echo -e ""
