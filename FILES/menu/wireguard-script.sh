...
function add-user() {
	...
	echo -e "$user\t$exp" >> /donb/wireguard/wireguard-clients.txt
	echo -e "[Interface]
...
Endpoint = ${endpoint}
AllowedIPs = 0.0.0.0/0" >> /donb/wireguard/${user}.conf
	echo -e "\n### Client ${user}
[Peer]
...
function delete-user(){
	...
	if grep -qw "^### Client ${user}\$" /etc/wireguard/wg0.conf; then
		...
		rm -f /donb/wireguard/${user}.conf
		sed -i "/\b$user\b/d" /donb/wireguard/wireguard-clients.txt
...
function extend-user() {
	...
	echo -e "---------------------"
	read -p "Username : " user
	if ! grep -qw "$user" /donb/wireguard/wireguard-clients.txt; then
		echo -e ""
		echo -e "User '$user' does not exist."
		echo -e ""
		exit 0
	...
	exp_old=$(cat /donb/wireguard/wireguard-clients.txt | grep -w $user | awk '{print $2}')
	...
	sed -i "/\b$user\b/d" /donb/wireguard/wireguard-clients.txt
	echo -e "$user\t$exp_new" >> /donb/wireguard/wireguard-clients.txt
...
function user-list() {
	...
	do
		...
		printf "%-17s %2s\n" "$user" "$exp_date"
	done < /donb/wireguard/wireguard-clients.txt
	total=$(wc -l /donb/wireguard/wireguard-clients.txt | awk '{print $1}')
	echo -e "-------------------------------"
...
function show-config() {
	...
	read -p "User : " user
	if grep -qw "^### Client ${user}\$" /etc/wireguard/wg0.conf; then
		exp=$(cat /donb/wireguard/wireguard-clients.txt | grep -w "$user" | awk '{print $2}')
		exp_date=$(date -d"${exp}" "+%d %b %Y")
		echo -e "Expired : $exp_date"
		echo -e ""
		echo -e "QR Code"
		echo -e "-------"
		qrencode -t ansiutf8 -l L < /donb/wireguard/${user}.conf
		echo -e ""
		echo -e "Config"
		echo -e "------"
		cat /donb/wireguard/${user}.conf
		echo -e ""
	else
		echo -e ""
		echo -e "User '$user' does not exist."
