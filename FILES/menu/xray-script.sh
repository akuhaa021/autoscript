...
function add-user() {
	...
	if grep -qw "$user" /donboyage/xray/xray-clients.txt; then
		echo -e ""
	...
	email=${user}@${domain}
	echo -e "${user}\t${uuid}\t${exp}" >> /donboyage/xray/xray-clients.txt
	...
	uuid=$(cat /donboyage/xray/xray-clients.txt | grep -w "$user" | awk '{print $2}')
	domain=$(cat /usr/local/etc/xray/domain)
	exp=$(cat /donboyage/xray/xray-clients.txt | grep -w "$user" | awk '{print $3}')
	...
	echo -e "TLS : tls"
	echo -e ""
	echo -e "Link : vless://$uuid@$domain:3646?type=ws&security=tls&path=%2fxray#WS_TLS-$domain"
	echo -e "QR : https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$uuid@$domain:3646?type=ws%26security=tls%26path=%2fxray%23WS_TLS-$domain"
	echo -e ""
	...
	echo -e ""
	echo -e "Link Digi    : vless://$uuid@plus.google.com.my.$domain:3646?headerType=none&type=tcp&encryption=none&flow=xtls-rprx-direct&security=xtls&sni=vault21.digi.com.my#vless_SG0_xtls_Digi_$user"
	echo -e "Link Umobile : vless://$uuid@wap.u.com.my.$domain:3646?headerType=none&type=tcp&encryption=none&flow=xtls-rprx-direct&security=xtls&sni=clubopen.pubgmobile.com.wap.u.com.my#vless_SG0_xtls_Umobile_$user"
...
function delete-user() {
	...
	mv -f /usr/local/etc/xray/config_tmp.json /usr/local/etc/xray/config.json
	sed -i "/\b$user\b/d" /donboyage/xray/xray-clients.txt
	service xray restart
...
function extend-user() {
	...
	uuid=$(cat /donboyage/xray/xray-clients.txt | grep -w $user | awk '{print $2}')
	exp_old=$(cat /donboyage/xray/xray-clients.txt | grep -w $user | awk '{print $3}')
...
function user-list() {
	...
	do
		...
		exp_date=$(date -d"${exp}" "+%d %b %Y")
		printf "%-17s %2s\n" "$user" "$exp_date"
	done < /donboyage/xray/xray-clients.txt
	total=$(wc -l /donboyage/xray/xray-clients.txt | awk '{print $1}')
	echo -e "-------------------------------"
...
function user-monitor() {
	data=($(cat /donboyage/xray/xray-clients.txt | awk '{print $1}'))
